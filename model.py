...
CALORIE_CSV_PATH = "C:/Users/USER/Desktop/project1/calorie.csv"  # Path to calorie CSV file
...
def load_images_and_labels(images_dir, calorie_df, img_size=IMAGE_SIZE):
    ...
    for food_name in os.listdir(images_dir):
        print(f"Checking folder: {food_name}")  # Debugging line
        food_path = os.path.join(images_dir, food_name)
        if food_name.lower() in calorie_df['Fruits'].values:  # Ensure lowercase comparison
            for image_file in os.listdir(food_path):
                if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                    try:
                        ...
                        calories.append(calorie_df.loc[calorie_df['Fruits'] == food_name.lower(), 'calorie'].values[0])  # Ensure lowercase
                    except Exception as e:
                        print(f"Error reading {image_file}: {e}")
        else:
            print(f"Skipping folder {food_name}, not in calorie CSV.")  # Debugging line
...
# Check for enough samples for R² score calculation
if len(y_cal_test) > 1:
...
print(f"Calorie Estimation R² Score: {calorie_r2}")
